...
def run():
    ...
    with urllib.request.urlopen("https://api.oireachtas.ie/v1/legislation?bill_status=Current&bill_source=Government,Private%20Member&date_start=" + "2018-09-12" + "&date_end=2099-01-01&limit=50&chamber_id=&lang=en") as url:
        data = json.loads(url.read().decode())

        if data["head"]["counts"]["billCount"] > 0:
            i = 0
            for i in range(data["head"]["counts"]["billCount"]):
    ...
    if final_updated_bills.empty == False:
        
        result = pd.merge(final_updated_bills,
